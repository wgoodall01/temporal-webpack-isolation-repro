...
import { Worker, bundleWorkflowCode, Runtime, DefaultLogger } from "@temporalio/worker";
...
async function main() {
  ...
  const cacheDecls = code.match(/(?:var|const|let) __webpack_module_cache__ = [^\n;]*/g) ?? [];
  ...
  try {
    ...
    const [first, second] = await worker.runUntil(async () => {
      const first = await env.client.workflow.execute<typeof stateProbe>("stateProbe", {
        workflowId: "probe-1",
        taskQueue,
      });
      const second = await env.client.workflow.execute<typeof stateProbe>("stateProbe", {
        workflowId: "probe-2",
        taskQueue,
      });
      return [first, second];
    ...
    console.log(`probe-1 observed leaked module state from: ${first.leakedFrom}`);
    console.log(`probe-2 observed leaked module state from: ${second.leakedFrom}\n`);
    ...
    if (second.leakedFrom !== null) {
      ...
      console.log("✅ Isolation intact: no module-level state leaked between executions.");
    }
  } finally {
    await env.teardown();
  }
